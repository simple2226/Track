...
import FollowerInstance from '../followers/FollowerInstance'
...
export default function Following() {
    ...
    useEffect(() => {
        const getFollowing = async () => {
            const url = `${domain}/accounts/following/${account?._id}`
            try {
                const reponse = await axios.get(url)
                setFollowing(reponse.data)
            } catch {}
        }
        getFollowing()
        document.getElementById('option').textContent = 'Following'
    }, [])
    ...
    return (
        ...
        :
            <div className={`flex flex-col gap-y-2 items-center justify-center relative mt-[70px] 700:mt-[80px] overflow-x-hidden overflow-y-auto min-h-[calc(100vh-70px)] 700:min-h-[calc(100vh-80px)] min-w-[360px] w-full py-5 overlap-scr`}>
                ...
                <h1 className='font-robotoSans text-[#bdbdbd] text-[3rem]'>no following</h1>
            </div>
    )
}